...
public final class GrahamScan {
	...
	public static void main(String[] args) {
		...
		// find the convex hull
		convexHull = GrahamScan.getConvexHull(points);

		for(java.awt.Point p : convexHull) {
		    System.out.println(p);
		}
		
		System.out.println(perimetar(convexHull));
		System.out.println(ploshtina(convexHull));
	}
	
	public static double perimetar(List<Point> lista){
		double rez = 0;
		
		Point prev = null;
		for(Point point : lista){
			if(prev!=null) rez += Math.sqrt((point.x - prev.x)*(point.x - prev.x) + (point.y - prev.y)*(point.y - prev.y));
			prev = point;
		}
		return rez;
	}
	
	public static double ploshtina(List<Point> lista){
		double rez = 0;
		
		Point prev = null;
		for(Point point : lista){
			if(prev!=null) rez += (prev.getX()*point.getY());
			prev = point;
		}
		rez += (prev.getX()*lista.get(0).getY());
		
		prev = null;
		for(Point point : lista){
			if(prev!=null) rez -= (point.getX()*prev.getY());
			prev = point;
		}
		rez = rez*0.5;
		return rez;
